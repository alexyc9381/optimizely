...
const sampleBusinessData = {
  ...
  monthlySales: [150000, 165000, 180000, 172000, 195000, 210000, 225000, 240000, 255000, 270000, 285000, 300000],

  // Marketing metrics
  advertisingSpend: [15000, 16500, 18000, 17200, 19500, 21000, 22500, 24000, 25500, 27000, 28500, 30000],

  // Customer engagement
  websiteTraffic: [45000, 48000, 52000, 49000, 55000, 58000, 61000, 64000, 67000, 70000, 73000, 76000],

  // Conversion metrics
  conversionRate: [2.3, 2.5, 2.8, 2.4, 3.1, 3.3, 3.5, 3.8, 4.0, 4.2, 4.4, 4.6],

  // Customer satisfaction scores
  customerSatisfaction: [4.2, 4.3, 4.1, 4.4, 4.5, 4.6, 4.3, 4.7, 4.8, 4.6, 4.9, 5.0],

  // Support ticket volume
  supportTickets: [120, 115, 135, 128, 110, 105, 98, 92, 88, 85, 82, 78],

  // Revenue by channel
  organicRevenue: [60000, 66000, 72000, 68000, 78000, 84000, 90000, 96000, 102000, 108000, 114000, 120000],
  paidRevenue: [45000, 49500, 54000, 51000, 58500, 63000, 67500, 72000, 76500, 81000, 85500, 90000],
  socialRevenue: [25000, 27500, 30000, 28000, 32500, 35000, 37500, 40000, 42500, 45000, 47500, 50000],

  // User engagement metrics
  avgSessionDuration: [4.2, 4.5, 4.8, 4.3, 5.1, 5.3, 5.5, 5.8, 6.0, 6.2, 6.4, 6.6],
  pagesPerSession: [3.1, 3.3, 3.6, 3.2, 3.8, 4.0, 4.2, 4.4, 4.6, 4.8, 5.0, 5.2],

  // A/B test results
  experimentGroupA: [2.1, 2.3, 2.0, 2.4, 2.6, 2.2, 2.5, 2.7, 2.3, 2.8, 2.9, 2.6],
  experimentGroupB: [2.8, 3.1, 2.9, 3.3, 3.5, 3.2, 3.6, 3.8, 3.4, 3.9, 4.1, 3.7],

  // Seasonal trend data
  seasonalSales: [200000, 180000, 160000, 140000, 155000, 175000, 220000, 250000, 280000, 260000, 240000, 320000]
};
...
export const AnalyticsExample: React.FC = () => {
  ...
  const runAdvancedAnalysis = async () => {
    ...
    try {
      ...
      const seasonalTrend = analyticsEngine.trendAnalysis(sampleBusinessData.seasonalSales);
      console.log('Seasonal Sales Trend:', seasonalTrend);

      // Customer Satisfaction Trend
      const satisfactionTrend = analyticsEngine.trendAnalysis(sampleBusinessData.customerSatisfaction);
      ...
      const analysisResults = {
        ...
        summary: {
          totalTests: 2,
          totalCorrelations: 2,
          significantCorrelations: [salesAdvertisingCorr, satisfactionTicketsCorr].filter(c => c.isSignificant).length,
          highPriorityInsights: insights.filter(i => i.significance === 'high').length,
    ...
    <div className="analytics-example">
      <div className="example-header">
        ...
        <p>
          ...
          of the Advanced Analytics Engine using real business data scenarios.
        </p>
        ...
        <div className="example-stats">
          {results && (
            <div className="stats-grid">
              ...
              <div className="stat-card">
                <h3>Correlations</h3>
                <div className="stat-value">{results.summary.significantCorrelations}/{results.summary.totalCorrelations}</div>
                <div className="stat-label">Significant Correlations</div>
              </div>

              <div className="stat-card">
                <h3>Model Accuracy</h3>
                <div className="stat-value">{(results.summary.regressionRSquared * 100).toFixed(1)}%</div>
                <div className="stat-label">R-Squared Value</div>
              </div>

              <div className="stat-card">
                <h3>Key Insights</h3>
                <div className="stat-value">{results.summary.highPriorityInsights}</div>
      ...
      <div className="example-content">
        ...
        <p>
          The interface below provides interactive access to all advanced statistical tools.
          The sample business data includes sales, marketing, and customer metrics from a typical SaaS business.
        </p>
      ...
      {results && (
        <div className="analysis-summary">
          ...
          <div className="summary-section">
            ...
            <ul>
              <li>
                ...
                {results.statisticalTests[0].isSignificant ? 'statistically significant' : 'no significant'} {' '}
                improvement over Group A (p = {results.statisticalTests[0].pValue.toFixed(4)})
              </li>
              <li>
                <strong>Sales Target Analysis:</strong> Monthly sales {' '}
                {results.statisticalTests[1].isSignificant ? 'significantly exceed' : 'do not significantly differ from'} {' '}
                the $200K target (t = {results.statisticalTests[1].statistic.toFixed(2)})
              </li>
            </ul>
          </div>
          ...
          <div className="summary-section">
            ...
            <ul>
              <li>
                ...
                {results.correlations[0].strength.replace('_', ' ')} {results.correlations[0].direction} correlation {' '}
                (r = {results.correlations[0].coefficient.toFixed(3)})
              </li>
              <li>
                <strong>Satisfaction ↔ Support Tickets:</strong> {' '}
                {results.correlations[1].strength.replace('_', ' ')} {results.correlations[1].direction} correlation {' '}
                (ρ = {results.correlations[1].coefficient.toFixed(3)})
              </li>
            </ul>
          </div>
          ...
          <div className="summary-section">
            ...
            <ul>
              <li>
                <strong>Sales Prediction Model:</strong> R² = {(results.regressions[0].rSquared * 100).toFixed(1)}% {' '}
                - {results.regressions[0].isSignificant ? 'Highly predictive' : 'Needs improvement'}
              </li>
              <li>
                <strong>Model Equation:</strong> <code>{results.regressions[0].equation}</code>
              </li>
              {results.regressions[0].outliers.length > 0 && (
                <li>
                  <strong>Outliers Detected:</strong> {results.regressions[0].outliers.length} data points require investigation
                </li>
              )}
            </ul>
          </div>
          ...
          <div className="summary-section">
            ...
            <ul>
              <li>
                <strong>Seasonal Sales:</strong> {results.trends.seasonalSales.trend} trend detected {' '}
                with {results.trends.seasonalSales.anomalies.indices.length} anomalies
              </li>
              <li>
                <strong>Customer Satisfaction:</strong> {results.trends.customerSatisfaction.trend} trend {' '}
                (strength: {results.trends.customerSatisfaction.strength.toFixed(3)})
              </li>
            </ul>
          </div>
          ...
          <div className="summary-section">
            ...
            <div className="insights-preview">
              {results.insights.slice(0, 3).map((insight: any, index: number) => (
                <div key={index} className={`insight-preview significance-${insight.significance}`}>
                  <div className="insight-title">{insight.title}</div>
                  <div className="insight-confidence">{(insight.confidence * 100).toFixed(0)}% confidence</div>
                </div>
              ))}
            </div>
          </div>
