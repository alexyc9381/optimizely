import { BarChart3, Pause, Play, Target, TrendingUp, Users } from 'lucide-react';
...
const ABTestingPage: React.FC = () => {
  ...
  const handleStatusChange = async (testId: string, newStatus: ABTest['status']) => {
    try {
      const updatedTest = await apiClient.updateABTest(testId, { status: newStatus });
      ...
      <div className='flex items-start justify-between mb-4'>
        <div className='flex-1'>
          ...
          <div className='flex items-center space-x-3 mb-3'>
            ...
            <span className='text-sm text-gray-500 bg-gray-100 px-2 py-1 rounded'>{test.industry}</span>
            <span className='text-sm text-gray-500'>Started {new Date(test.startDate).toLocaleDateString()}</span>
          </div>
        </div>
        ...
        <>
          <div className='grid grid-cols-2 md:grid-cols-4 gap-4 mb-4'>
            <div className='text-center p-3 bg-gray-50 rounded-lg'>
              ...
              <p className='text-lg font-bold text-gray-900'>{formatNumber(test.visitors)}</p>
            </div>
            <div className='text-center p-3 bg-blue-50 rounded-lg'>
              ...
              <p className='text-lg font-bold text-blue-900'>{formatPercentage(test.conversionRate.control)}</p>
            </div>
            <div className='text-center p-3 bg-green-50 rounded-lg'>
              ...
              <p className='text-lg font-bold text-green-900'>{formatPercentage(test.conversionRate.variant)}</p>
            </div>
            <div className='text-center p-3 bg-purple-50 rounded-lg'>
              ...
              <p className='text-lg font-bold text-purple-900'>{formatPercentage(test.confidence)}</p>
            </div>
          </div>
            ...
            <div className='bg-gradient-to-r from-green-50 to-green-100 border border-green-200 rounded-lg p-4'>
              <div className='flex items-center'>
                ...
                <span className='text-sm font-medium text-green-800'>
                  <strong>{formatPercentage(test.uplift)}</strong> improvement over control
                </span>
              </div>
              {test.confidence >= 95 && (
      ...
      <div className='space-y-6'>
          ...
          <div className='bg-white rounded-xl shadow-sm border border-gray-200 p-6'>
            <div className='flex items-center'>
              <div className='flex-1'>
                ...
                <p className='text-2xl font-bold text-gray-900'>
                  {tests.length > 0 ?
                    formatPercentage(tests.reduce((acc, test) => acc + test.confidence, 0) / tests.length, 0)
                    : '0%'
                  }
                </p>
              </div>
          ...
          <div className='bg-white rounded-xl shadow-sm border border-gray-200 p-6'>
            <div className='flex items-center'>
              <div className='flex-1'>
                ...
                <p className='text-2xl font-bold text-gray-900'>
                  {formatNumber(tests.reduce((acc, test) => acc + test.visitors, 0))}
                </p>
              </div>
        ...
        <div className='border-b border-gray-200'>
          <nav className='-mb-px flex space-x-8'>
            {[
              { key: 'active', label: 'Active Tests', count: tests.filter(t => t.status === 'Running' || t.status === 'Paused').length },
              { key: 'completed', label: 'Completed', count: tests.filter(t => t.status === 'Completed').length },
              { key: 'drafts', label: 'Drafts', count: tests.filter(t => t.status === 'Draft').length },
            ].map(tab => (
              <button
                key={tab.key}
        ...
        {filteredTests.length > 0 ? (
          ...
          <div className='bg-white rounded-lg border border-gray-200 p-12 text-center'>
            ...
            <p className='text-gray-500 mb-6'>
              {activeTab === 'active' && "You don't have any active tests. Create your first test to get started."}
              {activeTab === 'completed' && "No completed tests yet. Once your tests finish, they'll appear here."}
              {activeTab === 'drafts' && "No draft tests. Create a new test and save it as a draft."}
            </p>
