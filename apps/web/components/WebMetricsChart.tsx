import { ArrowLeft, ChevronDown, Clock, MousePointer, TrendingUp } from 'lucide-react';
...
const WebMetricsChart: React.FC = () => {
  ...
  const generateMockData = (metricType: string): MetricDataPoint[] => {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const trendFactor = metricType === 'bounce' ? (1 - i * 0.002) : (1 + i * 0.002); // Bounce rate decreases, others increase
              ...
              {Object.values(metrics).map((metric) => (
                <button
                  ...
                  className={`w-full flex items-center space-x-3 px-4 py-3 text-left hover:bg-gray-50 transition-colors first:rounded-t-lg last:rounded-b-lg ${
                    selectedMetric === metric.id ? 'bg-blue-50 text-blue-700' : 'text-gray-700'
                  }`}
                >
                  {metric.icon}
          ...
          <div className="h-64">
            <svg width="100%" height="100%" className="overflow-visible">
              ...
              {[0, 25, 50, 75, 100].map((percent) => {
                ...
                const displayValue = selectedMetric === 'traffic'
